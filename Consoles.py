...
columns = {'Platform': platform, 'Type': type, 'Firm': firm, 'Release': release, 'Units Sold (in Millions)': units_sold}

print (rec_count)

# Ensuring "units_sold" column is in numeric format
df = pd.DataFrame(columns)
df[["Units Sold (in Millions)"]] = df[["Units Sold (in Millions)"]].apply(pd.to_numeric)

print(df)
df = df[['Platform', 'Type', 'Firm', 'Release', 'Units Sold (in Millions)']]
df.index.name = None
